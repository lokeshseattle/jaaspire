...
export default function VideoEditorPage() {
  ...
  const { uri, fileName, width, height } = useLocalSearchParams<{
    uri: string;
    fileName: string;
    width?: string;
    height?: string;
  ...
  const fallbackDimensions = useMemo(() => {
    ...
    if (w && w > 0 && h && h > 0) return { width: w, height: h };
    return undefined;
  }, [width, height]);
  ...
  const handleConfirm = useCallback(
    (result: VideoEditorResult) => {
      ...
      storyMutation.mutate({
        fileUri: uri,
        fileName: fileName,
        trimStart: Math.round(result.startTime / 1000),
        trimEnd: Math.round(result.endTime / 1000),
      });
    ...
    [router, uri, fileName, storyMutation],
  );
